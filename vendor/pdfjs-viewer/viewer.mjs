...
;// ./web/pdf_filter_view.js
const FILTER_WIDTH = 98;
class PDFFilterView {
  constructor({
    container,
    eventBus,
...
class PDFFilterViewer {
  ...
  #setupMessageListeners() {
    window.addEventListener("message", event => {
      if (event.data && event.data.type === "pdfjs-highlights" && event.data.source === "pdf.js-client") {
        console.log("Received highlights from parent window:", event.data.data);
        this.setHighlights(event.data.data);
      }
    }, false);
    document.addEventListener("pdfhighlightsdatareceived", event => {
...
class FilterHighlightOverlay {
  ...
  #getPageView(pageNumber) {
    const pdfViewer = window.PDFViewerApplication?.pdfViewer || window.PDFViewerApplication?.pdfSidebar?.pdfViewer;
...
class PDFSidebar {
  ...
  switchView(view, forceOpen = false) {
    ...
    toggleCheckedBtn(this.filterButton, view === SidebarView.FILTER, this.filterView);
    if (view === SidebarView.FILTER) {} else if (view !== SidebarView.THUMBS) {
  ...
  #addEventListeners() {
    ...
    this.filterButton.addEventListener("click", () => {
      ...
      eventBus.dispatch("filterviewactivated", {
        source: this
      });
    });
...
const PDFViewerApplication = {
  ...
  async saveSelectionAsJson(params = null) {
    ...
    try {
      ...
      const range = selection.getRangeAt(0);
      const pageElement = range.startContainer.nodeType === Node.TEXT_NODE ? range.startContainer.parentElement?.closest(".page") : range.startContainer.closest?.(".page");
      if (!pageElement) {
...
function onUpdateViewarea({
  ...
  if (this.appConfig.secondaryToolbar) {
    this.appConfig.secondaryToolbar.viewBookmarkButton?.setAttribute("href", this.pdfLinkService.getAnchorUrl(location.pdfOpenParams));
  }
}
